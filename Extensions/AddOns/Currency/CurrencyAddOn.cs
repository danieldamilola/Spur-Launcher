...
public sealed class CurrencyAddOn : IAddOn
{
    ...
    public async Task<AddOnResult> ExecuteAsync(string input, CancellationToken ct = default)
    {
        ...
        if (!m.Success)
            return new AddOnResult { Success = false, Title = Name, Detail = "Invalid format" };
        ...
        try
        {
            ...
            return new AddOnResult { Success = false, Title = Name, Detail = "Currency not found" };
        }
        catch (Exception ex)
        {
            return new AddOnResult { Success = false, Title = Name, Detail = $"Error: {ex.Message}" };
        }
    }
}
