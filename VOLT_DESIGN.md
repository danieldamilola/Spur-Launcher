...
Volt is Spotlight brought to Windows — but not copied. It takes Spotlight's philosophy (one bar, total calm, results appear when needed), Raycast's information density and result row clarity, and adapts both to feel native on Windows 11.

The defining visual choice: **frosted glass**. The window has no solid background — it blurs whatever is behind it. On Windows 11, `backdrop-filter: blur()` is supported in WebView2. This makes Volt feel like it belongs to the OS rather than sitting on top of it. Spotlight does this on Mac. Volt does it on Windows.
...
**What makes Volt different from all three:**
...
This is the signature of Volt. Every surface is glass.
...
**Important:** The Tauri window must have `transparent: true` and `decorations: false`. The glass effect is applied to the root `#app` div — not to the OS window itself.

---

## Color Palette

Volt is monochrome-first. The only color in the UI is:
...
The search bar is the entire visual identity of Volt. It must feel premium.
...
These sit inside the search bar on the right side — exactly as shown in the reference image.
...
9. Version number — `Volt v0.1.0` in 11px `--muted`, right-aligned, bottom
