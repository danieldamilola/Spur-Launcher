# brand.md — Arc Identity

> Arc is not “Spotlight for Windows.” It is a **native-feeling command surface** with its own calm voice—borrowed discipline from Apple, owned visual character from the Arc mark.

---

## 1. Brand essence

| Pillar | Meaning for Arc |
|--------|------------------|
...
The launcher icon set (`arc-launcher-*.ico`) is the **only** full-color brand artifact in the product.
...
- **Stroke:** a single confident **arc**—bright, slightly warm highlight (think moonlit edge, not neon)
...
| **In-app glyph** | Use a **1.5px stroke** Lucide-style icon set, rounded caps—same weight as the arc stroke *feeling* |
| **No duplicates** | Don’t place the app icon next to every result; app icons come from the OS. |
| **Clear space** | Minimum padding around the mark = ½ arc radius in marketing; in UI, 8px. |

### Sampling accent from the icon (implementation)

1. Open `Icons/arc-launcher-256x256.ico` in a color picker.
2. Sample the **brightest stroke** along the arc → `Brand.Highlight`
...
| `Brand.Highlight` | `#E8E4DF` | Arc stroke family (warm pearl) |
...
- [ ] Tray icon matches `Icons/arc-launcher-16x16.ico`
